...
- RV delivery is limited to 150 km one way from the Kelowna base. The fee is CA$150 through 40 km, then CA$2.50/km in each direction (CA$5 total for the two-way journey per additional one-way kilometre).
...
- Every RV quote includes mandatory `Stationary Plus Protection` at a fixed CA$150 per trip, regardless of the number of nights. Both mandatory charges must appear as separate quote line items, and backend quote code is the pricing source of truth.
