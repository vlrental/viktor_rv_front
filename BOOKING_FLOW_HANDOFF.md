...
- Policy: maximum 150 km one way from Kelowna; CA$150 through 40 km, then CA$2.50/km in each direction (CA$5 total for the two-way journey per additional one-way kilometre).
...
- The summary sidebar mirrors the step-5 Terms checkbox directly below the payment-mode notice so either control records the same required acceptance before reservation creation. The Terms link opens the agreement as a nested topmost layer; closing it with the visible control, `Back to payment`, backdrop, or `Escape` returns to the unchanged booking/payment state. Secure Checkout also requires its payment-layer confirmation before embedded Stripe mounts.
...
- Backend Stripe requests are pinned to API version `2025-06-30.basil`; all Checkout Session creations include the stable integration identifier `viktor_rv_qpmxkzrt`. The production deployment gate creates and expires a real embedded test Checkout against this exact contract. Configure and test the Dashboard webhook endpoint against the same API contract before any separately approved deployment or live activation. Prefer a least-privilege restricted test key (`rk_test_...`); standard test secret keys remain supported for compatibility.
