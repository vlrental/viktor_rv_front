...
fn RvEditorPanel(
    ...
    let selected_photo_label = photo_file
        .read()
        .as_ref()
        .map(|file| format!("Selected: {} · {:.0} KB", file.name(), file.size() / 1024.0));
    rsx! {
        section { class: "admin-panel admin-rv-editor-panel", role: "region", aria_label: if is_new { "Add RV" } else { "Edit RV" }, tabindex: "-1", autofocus: true, onkeydown: move |event| { event.stop_propagation(); if event.key()==Key::Escape { request_close(); } },
                ...
                div { class: "admin-drawer-scroll admin-rv-editor-scroll",
                    ...
                    if let Some(value)=detail {
                        section { class:"admin-drawer-section", h3 { "Photos ({value.media.len()}/40)" }
                            div { class:"admin-rv-photo-upload", label { "Image file" input { r#type:"file", disabled:busy()||value.media.len()>=40, accept:"image/jpeg,image/png,image/webp", oninput:move|event|{for file in event.files(){if file.size()>10*1024*1024{message.set(format!("{} is larger than 10 MB.",file.name()));continue}photo_file.set(file.inner().downcast_ref::<web_sys::File>().cloned());if photo_alt().trim().is_empty(){photo_alt.set(format!("{} exterior",name().trim()))}message.set("Photo selected. Upload it before publishing the RV.".into());photo_draft_dirty.set(true);aux_dirty.set(true);break}} } if let Some(label)=selected_photo_label.as_ref(){ small { class:"admin-photo-selected", "{label}" } } } label { "Alt text" input { value:"{photo_alt}", disabled:busy(), placeholder:"Describe what is visible in the photo", oninput:move|e|{photo_alt.set(e.value());photo_draft_dirty.set(true);aux_dirty.set(true)} } } label { class:"admin-check-field", input { r#type:"checkbox", checked:photo_cover(), disabled:busy(), onchange:move|e|{photo_cover.set(e.checked());photo_draft_dirty.set(true);aux_dirty.set(true)} } "Cover photo" } button { r#type:"button", disabled:busy()||value.media.len()>=40||photo_file.read().is_none()||photo_alt().trim().is_empty(), onclick:{let id=rental_id.clone();move |_|{let Some(file)=photo_file.read().clone()else{return};let alt=photo_alt();let cover=photo_cover();let id=id.clone();busy.set(true);photo_uploading.set(true);message.set("Uploading photo…".into());let _=spawn(async move{match api::upload_admin_rental_media(&id,&file,&alt,cover,next_media_order).await{Ok(_)=>{photo_file.set(None);photo_alt.set(String::new());photo_draft_dirty.set(false);aux_dirty.set(feature_draft_dirty()||addon_draft_dirty());refresh_rental_editor_after_change(&id,message,on_changed).await;if message().is_empty()||message()=="Uploading photo…"{message.set("Photo uploaded. The RV can now be published.".into())}},Err(error)=>message.set(error.message)}photo_uploading.set(false);busy.set(false)});}}, if photo_uploading(){"Uploading…"}else{"Upload photo"} } }
                ...
                footer { class:"admin-drawer-actions", button { r#type:"button", disabled:busy()||!dirty(), onclick:move |_|save(), if photo_uploading(){"Uploading photo…"}else if busy(){"Saving…"}else{"Save changes"} }
                    if !is_new { button { r#type:"button", class:if published{"danger"}else{""}, disabled:busy()||dirty()||aux_dirty(), title:if photo_draft_dirty(){"Upload the selected photo before publishing"}else if dirty()||aux_dirty(){"Save or discard draft changes first"}else{""}, onclick:{let id=rental_id.clone();move |_|{let action=if published{"archive"}else{"publish"};let prompt=if published{"Archive this RV? Existing bookings remain unchanged."}else{"Publish this RV? It will immediately appear in booking."};if !web_sys::window().and_then(|w|w.confirm_with_message(prompt).ok()).unwrap_or(false){return}let id=id.clone();busy.set(true);let _=spawn(async move{match api::admin_rental_publication_action(&id,action).await{Ok(value)=>on_changed.call(value),Err(error)=>message.set(error.message)}busy.set(false)});}}, if published{"Archive RV"}else{"Publish RV"} } }
